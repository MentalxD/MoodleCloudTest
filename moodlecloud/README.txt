...
To restore this backup you should be familiar with the MySQL database.

Step 1: The Database
  Restore the database dump file (niclas_moodlecloud.sql) to your mysql server using the `mysql` command
  Note: you will need to create the database before hand.
  Example: CREATE DATABASE database_name DEFAULT CHARACTER SET 'utf8mb4' DEFAULT COLLATE utf8mb4_bin;
  Example: mysql -h database_host -u database_user -p database_name < niclas_moodlecloud.sql
...
Step 3: Get The Moodle Code
  ...
  Example: git clone -b MOODLE_45_STABLE git://git.moodle.org/moodle.git
